...
Last synchronized: 2026-05-28 (Agent 2 prompt reduction synced)
...
- `/api/question` and Agent 2 are connected in `app/api/question/route.ts`, `lib/agents/questioner.ts`, and `tests/lib/questioner.test.ts`: Agent 2 returns natural replies plus structured level state, normal/hint/answer are distinguished, malformed output falls back safely, and model-provided pass status is ignored unless there is a real user answer. Agent 2 now uses a reduced knowledge-check tutor prompt: give the LLM the role, task boundary, level standards, and JSON contract, but avoid scenario playbooks. Fallback is intentionally plain and does not try to classify user intent. Code only filters frontend flow-control button text before accepting model-provided pass status.

Direction note: this change is subtraction, not addition. If future work wants to add "when X happens, say Y" rules to Agent 2, reject that first and prefer restoring LLM freedom under the tutor goal.
- `/exam` level-based client flow is complete in `app/exam/page.tsx`, `app/exam/_hooks/useQuestionFlow.ts`, `lib/examFlow.ts`, `store/examStore.tsx`, `tests/app/useQuestionFlow.test.tsx`, `tests/lib/examFlow.test.ts`, and `tests/store/examStore.test.ts`: the page uses Agent 2 `nextAction` instead of fixed answer counts, waits for the quick-path complete/deep-dive choice, wires hint/answer requests through `/api/question`, and blocks empty answer submission. User answers, hint clicks, and answer clicks are written to the node dialogue before the Agent request starts, so the conversation reacts immediately and the request history includes the just-submitted user turn. Agent 2 responses are written back by request-time `nodeId`, so switching knowledge points while a request is pending no longer contaminates the active dialogue. Completed knowledge points are marked in `nodePathStates` and receive a completion message in the dialogue. Deep path currently auto-covers analysis/evaluation only; creation is not entered automatically.
