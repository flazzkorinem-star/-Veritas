...
- `src/config/knowledge-map-limits.ts`：定义材料模块、知识条目、诊断主题总量与每个最终诊断主题最多五个条目的统一容量边界，供模型契约、本地报告与存储复用。
...
- `src/domain/agents/operation-meta.ts`：校验同源 Agent 成功响应中的尝试次数、结构修复标记和模型验证来源。
...
- `src/features/materials/agent-client.ts`：从浏览器调用同源 Agent 路由，要求并校验稳定结果与不含正文的操作元数据。
...
- `e2e/agent2-no-answer-real.spec.ts`：显式启用时在生产 Edge 中用真实 DeepSeek 区分六种无法作答表达、用户提问、暂停和带尝试的回答；同时验证非正确反馈不会单轮或累计补齐答案、部分回答不会被 Vita 补全、旧版泄露内容的复述不能独立通过，以及真实首问的三轮停滞、自动答案、同层小验证、答对后推进、分数与四层持久化状态。
